...
const { JOINABLE_STATES } = lightConfig;
...
class Lighting {
  ...
  async onStateChange(from, to, gameColor) {
    ...
    try {
      ...
      // 'Off' is handled even when its row is switched off, because putting
      // the lights back is governed by its own setting. Otherwise turning
      // the row off -- which reads as "don't switch my lights off" -- also
      // silently cancelled the restore, and the lights stayed on the PS5's
  ...
  async reconcile(prev, next, derivedState, gameColorHex) {
    ...
    // A light taken out of the bridge's control -- its own switch turned
    // off, every state unticked, or removed from the selection -- has to be
    // handed back straight away. Leaving it on the PS5's colours makes the
    // switch look like it did nothing.
    await this.releaseDropped(prev, next);
  ...
  // Hands specific lights back: replays their recorded state and drops them
  // from the snapshot, leaving the rest of the session's record intact.
  async releaseDropped(prev, next) {
    const before = this.controllableLights(prev);
    if (!before.length) return;
    const after = new Set(this.controllableLights(next));
    const dropped = before.filter((id) => !after.has(id));
    if (!dropped.length) return;
    if (await snapshot.restore(SESSION, 1, dropped)) {
      log(`Released ${dropped.join(", ")}`);
    }
  }

  // Records the lights the first time the bridge is about to touch them,
  ...
  async captureBeforeTouching(config) {
    ...
    // Merges, so lights already recorded keep their original entry and a
    // light added to the selection mid-session is picked up here.
    await this.snapshot(this.controllableLights(config));
  }
  ...
  async stopControlling(config) {
    ...
    const targets = this.controllableLights(config);
    if (!targets.length) return;
    await hass.callService("light", "turn_off", {
      entity_id: targets,
      transition: 1,
    });
    snapshot.clearApplied();
    log("Lighting released, lights turned off");
  }

  async snapshot(lights) {
    if (lights.length) await snapshot.capture(SESSION, lights);
  }
  ...
  async handleOff(config, stateConfig) {
    ...
    if (stateConfig.enabled !== false && stateConfig.turnOff !== false && targets.length) {
      await hass.callService("light", "turn_off", {
        entity_id: targets,
  ...
  // Lights the bridge is allowed to touch at all. A light switched off here,
  // or sitting out every state, must never be snapshotted or restored --
  // doing so reaches into a light it was told to leave alone and undoes
  // whatever the user did to it during the session.
  controllableLights(config) {
    return (config.lights || []).filter((id) => {
      const per = this.settingsFor(config, id);
      return per.enabled !== false && JOINABLE_STATES.some((k) => per.states?.[k] !== false);
    });
  }
  ...
  async applyState(config, stateConfig, gameColor, stateKey) {
    ...
    for (const entityId of targets) {
      ...
      // Remember what we sent, so this light can't later be mistaken for the
      // user's own lighting and snapshotted as if it were.
      snapshot.noteApplied(entityId, data);
