...
// Dark covers are the awkward case. Their dominant hue family is often only
// faintly coloured, so taking its saturation at face value produces a
// washed-out grey that looks nothing like the game (Demon's Souls came out
// #567181, a flat slate). Lifting saturation to a floor fixes that.
//
// The opposite mistake is just as easy: scoring families by vividness
// instead of size pushes muted art to neon (The Last of Us went from a
// faithful olive to #00ff24). So family choice stays size-weighted, and only
// the *output* saturation gets a floor.
const SAT_FLOOR = 0.45;
const MIN_LIGHTNESS = 0.08; // low enough to see accents in dark art
...
function dominantColor(pixels, width, height) {
  ...
  const s = Math.max(SAT_FLOOR, percentile(sats, 0.75));
  // Track the art's own lightness rather than pinning everything to the
  // middle, so a dark game still reads darker than a bright one -- but stay
  // above ~0.38, since below that an LED just looks off rather than coloured.
  const l = Math.min(0.6, Math.max(0.38, 0.28 + percentile(lights, 0.5) * 0.6));
