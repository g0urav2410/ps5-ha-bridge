...
function startServer(psn, port, ps5Ip, lighting) {
  ...
  app.put("/api/lighting", async (req, res) => {
    try {
      const prev = lightConfig.load();
      const saved = lightConfig.save(req.body);
      res.json(saved);
      // After responding, so the panel stays snappy while the lights catch up.
      const snap = sharedState.snapshot();
      lighting
        .reconcile(prev, saved, snap.derivedState, snap.gameColor)
        .catch(() => {});
    } catch (err) {
  ...
  app.post("/api/lighting/preview", async (req, res) => {
    ...
    try {
      await lighting.beginPreview(config);
      const snap = sharedState.snapshot();
      ...
      if (state === "off") {
        ...
        await lighting.applyState(config, stateConfig, gameColor, state);
      }
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({ error: err.message });
  ...
  // Put the lights back the way they were before previewing.
  app.post("/api/lighting/preview/stop", async (req, res) => {
    try {
      const restored = await lighting.endPreview(lightConfig.load());
      res.json({ restored });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
