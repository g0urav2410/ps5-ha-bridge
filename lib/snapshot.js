...
// Records each light exactly as it is right now. Merges: a light already
// recorded keeps its original entry, so this can be called on every state
// change without the first record being overwritten by later ones. That is
// also what picks up a light added to the selection mid-session, which used
// to finish the session stuck on the PS5's colours.
async function capture(key, lights) {
  if (!lights?.length) return false;
  try {
    const existing = read(key);
    const saved = existing ? existing.lights.slice() : [];
    const known = new Set(saved.map((e) => e.entity_id));
    const fresh = lights.filter((id) => !known.has(id));
    if (!fresh.length) return saved.length > 0;
    ...
    write(key, { at: existing?.at || Date.now(), lights: saved });
...
// on get their own brightness, colour and effect again. `only` limits this
// to some of the recorded lights, leaving the rest recorded -- that's what
// hands back a single light taken out of the bridge's control mid-session.
async function restore(key, transition = 1, only = null) {
  const snap = read(key);
  if (!snap) return false;
  const wanted = only ? snap.lights.filter((e) => only.includes(e.entity_id)) : snap.lights;
  ...
  if (only) {
    const rest = snap.lights.filter((e) => !only.includes(e.entity_id));
    write(key, rest.length ? { ...snap, lights: rest } : null);
    const applied = read("applied") || {};
    for (const id of only) delete applied[id];
    write("applied", Object.keys(applied).length ? applied : null);
    log(`${wanted.length} light(s) handed back`);
    return true;
  }
  write(key, null);
